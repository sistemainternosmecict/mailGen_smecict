...
class GoogleAdminDirectory:
    ...
    def authenticate(self):
        ...
        ).with_subject("thyezoliveiramonteiro@smec.saquarema.rj.gov.br")
        return creds
    ...
    def get_user_by_fullname(self, full_name):
        results = (
            self.service.users().list(customer="my_customer", maxResults=10).execute()
        )
        users = results.get("users", [])
        for user in users:
            print(f"{user['primaryEmail']} ({user['name']['fullName']})")
    ...
    def get_email_by_fullname(self, full_name):
        results = (
            self.service.users().list(customer="my_customer", maxResults=100).execute()
        )
        users = results.get("users", [])

        # Normaliza o nome de busca (remove acentos e converte para minúsculas)
        normalized_search_name = (
            unicodedata.normalize("NFD", full_name.lower())
            .encode("ASCII", "ignore")
            .decode("ASCII")
        )
    ...
    def update_user(
        ...
        try:
            updated_user = (
                self.service.users()
                .update(userKey=primary_email, body=user_info)
                .execute()
            )
    ...
    def prepare_users_from_xlsx(self):
        ...
        for aluno in alunos:
            ...
            org_unit_path = "/ESCOLAS/Escola Municipal Luciana Santana Coutinho"
    ...
    def reset_passwords_from_xlsx(
        self, new_password="provisorio2026", force_change_at_login=True
    ):
        if not os.path.exists(self.file_path):
            print("Arquivo não encontrado:", self.file_path)
            return
        ...
        if df.empty or "mat" not in df.columns:
            print("Arquivo não possui uma coluna 'matricula'.")
            return
        ...
        for idx, row in df.iterrows():
            mat = row["mat"]
            email = f"{mat}@smec.saquarema.rj.gov.br"
            if pd.notna(email):
                try:
                    self.update_user(
                        primary_email=email,
    ...
    def criar_sala(self, parent: str, nome_sala: str) -> None:
        body = {"name": nome_sala, "parentOrgUnitPath": parent}

        try:
            orgunit = (
                self.service.orgunits()
                .insert(customerId="my_customer", body=body)
                .execute()
            )

            print(f"✅ Sala criada: {orgunit['orgUnitPath']}")

        except HttpError as e:
            if e.resp.status == 409:
                print(f"⚠️ Sala já existe: {nome_sala}")
            else:
